Feature: Log In as Student
  ...
  Scenario: Attempt to log in with invalid username (Error Flow)
  ...
  Scenario: Attempt to log in with invalid password (Error Flow)

    The student should not be logged in and see an error message.

    Given a student account with <username> and <password> exists
    When student enters valid username <username>
    And student enters a wrong password <input_password>
    And student requests to log in
